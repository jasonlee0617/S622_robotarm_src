...
from yolo_perception_utils.visualization import draw_detection_diagnostics  # noqa: E402
...
def test_llm_perception_uses_the_new_result_topics_only():
    source = (ROOT / "yolo_perception" / "nodes" / "llm_yolo_perception.py").read_text()
    ...
    assert not (ROOT / "yolo_perception_utils" / "llm_yolo_perception.py").exists()
    assert (
        "ament_python_install_package(yolo_perception_nodes PACKAGE_DIR yolo_perception/nodes)"
        in cmake
    )


def test_llm_perception_entrypoint_is_executable():
    assert os.access(ROOT / "yolo_perception" / "nodes" / "llm_yolo_perception.py", os.X_OK)
...
def test_llm_perception_supports_continuous_or_search_gated_inference():
    ...
    assert "/llm_yolo_perception/set_inference_enabled" in source
    assert "/llm_yolo_perception/release_gpu" in source
...
def test_obb_detector_uses_the_same_inference_gate_contract():
    source = (ROOT / "yolo_perception" / "nodes" / "yolo_detector_obb.py").read_text()
