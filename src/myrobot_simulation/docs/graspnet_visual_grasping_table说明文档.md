...
本文档说明 `myrobot_simulation/launch/graspnet_visual_grasping_gazebo.launch.py` 的用途、启动前准备、启动命令、运行链路、验收方法和常见问题。该 launch 面向 Gazebo 仿真下的 Fairino Arm 机械臂 GraspNet 视觉抓取，目标是完成最小闭环：
...
ros2 launch myrobot_simulation graspnet_visual_grasping_gazebo.launch.py
```
...
| 组件 | 作用 |
| --- | --- |
| `myrobot_simulation/launch/gazebo_yolo.launch.py` | 启动 Gazebo、Fairino Arm、MoveIt、RViz、相机模型和相机 bridge。 |
...
cd $HOME/fairino_robotarm
source /opt/ros/humble/setup.bash
colcon build --packages-select graspnet_grasping myrobot_simulation trajectory_retime_server hand_eye_calibration yolov8_grasping
source install/setup.bash
```
...
如果这些路径不存在，需要先修正 `myrobot_simulation/launch/graspnet_visual_grasping_gazebo.launch.py` 中的 `conda_setup`、`baseline_dir` 或 `checkpoint_path`。
...
cd $HOME/fairino_robotarm
source /opt/ros/humble/setup.bash
colcon build --packages-select graspnet_grasping myrobot_simulation --symlink-install
source install/setup.bash
```
...
python3 -m py_compile \
  myrobot_simulation/launch/graspnet_visual_grasping_gazebo.launch.py \
...
cd $HOME/fairino_robotarm
source /opt/ros/humble/setup.bash
colcon build --packages-select graspnet_grasping myrobot_simulation
```
