# graspnet_grasping_gazebo.launch.py 说明文档

本文档说明 `myrobot_simulation/launch/graspnet_grasping_gazebo.launch.py` 的用途、启动前准备、启动命令、运行链路、验收方法和常见问题。该 launch 面向 Gazebo 仿真下的 Fairino Arm 机械臂 GraspNet 视觉抓取，目标是完成最小闭环：
...
ros2 launch myrobot_simulation graspnet_grasping_gazebo.launch.py
```
...
如果这些路径不存在，需要先修正 `myrobot_simulation/launch/graspnet_grasping_gazebo.launch.py` 中的 `conda_setup`、`baseline_dir` 或 `checkpoint_path`。
...
`graspnet_grasping_gazebo.launch.py` 固定使用包内执行节点 YAML。`graspnet_visual_grasping` 执行节点参数优先级为：
...
python3 -m py_compile \
  myrobot_simulation/launch/graspnet_grasping_gazebo.launch.py \
