...
**功能：** 为 visual_servo_bringup, yolo_bringup, hand_eye_calibration, llm_arm_control 等包提供共享基础能力，避免代码重复。
...
#### [yolo_bringup](yolo_bringup/) — YOLO 视觉抓取执行
...
**入口点：**
- `visual_servo_grasping` — 基于 YOLO OBB 的闭环位置伺服装夹抓取
...
| `graspnet_grasping_gazebo.launch.py` | Gazebo GraspNet 抓取 |
| `visual_position_servo_gazebo.launch.py` | 位置伺服仿真 |
| `calibration_gazebo.launch.py` | 标定仿真 |
| `ik_test_demo_gazebo.launch.py` | IK 测试 |
| `trajectory_plan_demo_gazebo.launch.py` | 轨迹规划演示 |
| `llm_robot_control_gazebo.launch.py` | LLM + YOLO 终端确认控制仿真 |
...
感知链路:
┌───────────┐    ┌──────────────┐    ┌────────────────────┐
│ Realsense │───▶│yolo_perception│───▶│ yolo_bringup    │
