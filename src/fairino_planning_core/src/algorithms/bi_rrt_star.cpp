...
// BiRRT* bidirectional RRT* with orientation fallback
...
PlanResult BiRRTStar::planWithFallback(
    ...
    const auto base_seed = static_cast<std::mt19937::result_type>(
        request_seed == 0 ? 7U : request_seed);
    ...
    best_result.failure_code = PlanningFailureCode::kGoalNotReached;
    best_result.message = "BiRRT* failed after all fallback passes.";
    return best_result;
}
