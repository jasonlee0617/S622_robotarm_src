...
#include "myrobot_planning_core/dh_kinematics.h"   // DH 参数正运动学库
#include "myrobot_planning_core/types.h"            // 关节配置、变换类型
#include <algorithm>
...
std::vector<Vec3> RobotKinematics::getJointPositions(const VecN& q,
                                                      ToolModel tool_model) const {
    // 1) 转换关节向量格式
    const fairino_planning::JointConfig q_fp = toJointConfig(q);

    // 2) 调用正运动学，获取全部变换 (T00..T06)
    auto all_T = fk_.fkineAll(q_fp);  // 返回 std::array<Transform4d, 7>
    ...
    if (tool_model == ToolModel::GRIPPER) {
        fairino_planning::Transform4d T_tool =
            all_T[6] * fk_.toolTransform(tool_model);
