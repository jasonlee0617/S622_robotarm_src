...
#include "myrobot_mpc_avoidance/nmpc_solver.hpp"
...
NMPCSolver::NMPCSolver() = default;

NMPCSolver::~NMPCSolver() {
    if (capsule_) {
        fairino_arm_nmpc_acados_free(capsule_);
        fairino_arm_nmpc_acados_free_capsule(capsule_);
    }
}
 ...
 */
bool NMPCSolver::initialize(const MPCParams& params) {
    // 保存初始参数快照，运行时可调用 updateParams() 覆盖
    params_ = params;

    // 创建 acados capsule
    capsule_ = fairino_arm_nmpc_acados_create_capsule();
    int status = fairino_arm_nmpc_acados_create(capsule_);
    if (status != 0) {
        return false;
    }

    // 获取内部接口指针
    nlp_config_ = fairino_arm_nmpc_acados_get_nlp_config(capsule_);
    nlp_dims_   = fairino_arm_nmpc_acados_get_nlp_dims(capsule_);
    nlp_in_     = fairino_arm_nmpc_acados_get_nlp_in(capsule_);
    nlp_out_    = fairino_arm_nmpc_acados_get_nlp_out(capsule_);
    nlp_solver_ = fairino_arm_nmpc_acados_get_nlp_solver(capsule_);
    nlp_opts_   = fairino_arm_nmpc_acados_get_nlp_opts(capsule_);
...
void NMPCSolver::resetSolverMemory(bool reset_qp_solver_mem) {
    if (!initialized_ || !capsule_) {
        return;
    }
    fairino_arm_nmpc_acados_reset(capsule_, reset_qp_solver_mem ? 1 : 0);
}
...
MPCResult NMPCSolver::solve(const MPCSolveContext& ctx)
{
    /*
     * 求解流程:
     * 1) 设置初始状态约束 x0
    ...
    for (int k = 0; k < N; ++k) {
        ...
        // 更新 acados 阶段参数
        fairino_arm_nmpc_acados_update_params(capsule_, k, p_vec.data(), np);
    ...
    // 终端阶段：使用 terminal_weight，无 APF/CBF 耦合
    {
        ...
        fairino_arm_nmpc_acados_update_params(capsule_, N, p_vec.data(), np);
    }

    // 4. 调用 acados 求解器
    int acados_status = fairino_arm_nmpc_acados_solve(capsule_);
    ...
    if (acados_status != 0) {
        ...
        if (log_callback_ && (fail_streak <= 3 || fail_streak % 100 == 0)) {
            ...
            snprintf(buf, sizeof(buf),
                "[NMPC-FAIL] failure #%d status=%d solve_t=%.1fms",
        ...
        if (log_callback_ && fail_streak >= 3) {
            char buf[256];
            snprintf(buf, sizeof(buf),
                "[NMPC-RECOVER] after %d consecutive failures, status=0 t=%.1fms",
    ...
    if (dbg_solve) {
        ...
        if (log_callback_) {
            char buf[512];
            snprintf(buf, sizeof(buf),
                "[NMPC] #%d h0=%.3f h_min=%.3f d_raw=%.3f APFth=%.3f APFpred=%.3f APFref=%.3f APFmax=%.3f |ddq|=%.1f trkW_eff=%.1f obsW_eff=%.1f velW_eff=%.2f termW_eff=%.1f trkS=%.2f obsS=%.2f velS=%.2f termS=%.2f st=%d t=%.1fms",
 ...
 */
void NMPCSolver::computeCBFParams(
 ...
 */
void NMPCSolver::computeAPFParams(
    const RefWindow& ref_win,
    const std::vector<std::vector<Obstacle>>& obs_pred,
    const std::vector<VecN>& q_pred,
 ...
 * return 最小裕度（可能为负）
 */
double NMPCSolver::computeMinMarginAtQ(const VecN& q_center,
 ...
 * return 势场值 J
 */
double NMPCSolver::computeAPFValue(const VecN& q, const std::vector<Obstacle>& obs) const {
 ...
 * return 梯度向量
 */
VecN NMPCSolver::computeAPFGradient(const VecN& q,
 ...
 */
void NMPCSolver::computeAdaptiveWeights(double margin,
 ...
 */
double NMPCSolver::computeSpeedRatio(double margin) const {
...
double NMPCSolver::computeRobotObsMargin(const VecN& q,
 ...
 */
Obstacle NMPCSolver::propagateDynamicObs(const Obstacle& obs, double dt) {
    Obstacle out = obs;
 ...
 */
std::vector<std::vector<Obstacle>> NMPCSolver::predictObs(
 ...
 */
std::vector<std::vector<Obstacle>> NMPCSolver::predictObsInternal(
