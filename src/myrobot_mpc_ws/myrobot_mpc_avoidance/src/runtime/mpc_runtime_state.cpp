...
void MpcRuntimeState::resetForNewTrajectory(bool preserve_replan_session) {
    std::lock_guard<std::mutex> lock(mutex_);
    ...
    state_.last_deadlock_check_time_sec = -1.0;
    state_.last_deadlock_margin = 0.0;
    state_.local_recovery_start_time_sec = -1.0;
    state_.static_block_start_time_sec = -1.0;
    state_.dynamic_clear_since_time_sec = -1.0;
    state_.waiting_for_replan = false;
    state_.waiting_dynamic_clearance = false;
    state_.terminal_hold = false;
    state_.reference_precheck_pending = true;
    if (!preserve_replan_session) {
        state_.replan_count = 0;
        state_.last_replan_step = -1000000;
        state_.last_replan_time_sec = -1.0;
        state_.replan_cooldown_until_sec = -1.0;
    }
