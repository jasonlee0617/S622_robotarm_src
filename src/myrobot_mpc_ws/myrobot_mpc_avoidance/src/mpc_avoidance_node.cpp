...
// 项目内部模块
#include "myrobot_mpc_avoidance/solver_selector.hpp"         // 求解器选择层
#include "myrobot_mpc_avoidance/mpc_params_loader.hpp"       // 参数加载
#include "myrobot_mpc_avoidance/scenario_loader.hpp"         // 障碍物场景加载
#include "myrobot_mpc_avoidance/obstacle_tracker.hpp"        // 动态障碍物跟踪
#include "myrobot_mpc_avoidance/arc_path_follower.hpp"       // 弧长路径跟随
#include "myrobot_mpc_avoidance/smooth_box_distance.hpp"     // 平滑盒子距离计算
#include "myrobot_mpc_avoidance/robot_kinematics.hpp"        // 机器人运动学
#include "myrobot_mpc_avoidance/rviz_visualizer.hpp"         // RViz可视化
#include "myrobot_mpc_avoidance/runtime/runtime_state.hpp"   // 运行时状态结构体
#include "myrobot_mpc_avoidance/runtime/mpc_runtime_state.hpp" // 线程安全运行时状态持有者
#include "myrobot_mpc_avoidance/control/control_coordinator.hpp" // 控制周期阶段协调器
#include "myrobot_mpc_avoidance/control/deadlock_replan_engine.hpp" // 死锁与重规划引擎
#include "myrobot_mpc_avoidance/control/command_pipeline.hpp" // 指令管道（偏置辅助、刹车等）
#include "myrobot_mpc_avoidance/control/scene_obstacle_provider.hpp" // 场景静态障碍物提供
...
public:
    ...
    MPCAvoidanceNode() : Node("mpc_avoidance_node") {
        ...
        declare_parameter("robot_profile", std::string("fairino_arm_gripper_onbase"));
        const auto robot_profile = get_parameter("robot_profile").as_string();
        if (robot_profile != "fairino3_v6" && robot_profile != "fairino_arm_gripper_onbase") {
            throw std::runtime_error(
                "robot_profile must be 'fairino3_v6' or 'fairino_arm_gripper_onbase'.");
        }
        ...
        if (enable_moveit_scene) {
            ...
            tcp_init_timer_ = create_wall_timer(std::chrono::milliseconds(0), [this]() {
                tcp_init_timer_->cancel();
                initMoveItScene();
            });
        } else {
            throw std::runtime_error(
                "enable_moveit_scene must be true: MPC requires robot_description to load tool0 TCP.");
        }
        ...
        RCLCPP_INFO(get_logger(),
            "Selected avoidance algorithm: %s, profile=%s, dt=%.4fs, rate=%.1fHz, N=%d",
...
private:
    ...
    void initMoveItScene() {
        try {
            ...
            if (!robot_model) {
                throw std::runtime_error("robot_description did not produce a RobotModel");
            }
            ...
            if (!tool_link || !tool_link->getParentLinkModel() ||
                ...
                throw std::runtime_error(
                    "robot_description must contain a fixed wrist3_link -> tool0 TCP chain");
            }
            fairino_planning::Transform4d wrist3_to_tool = fairino_planning::Transform4d::Identity();
            ...
            const auto flange_to_tool = fairino_planning::DHKinematics::flangeToToolTransform(
                fairino_planning::DHParams{}, wrist3_to_tool);
            kinematics_ = RobotKinematics(flange_to_tool);
            solver_->setKinematics(kinematics_);
            rviz_visualizer_->setKinematics(kinematics_);
            tcp_ready_ = true;
            RCLCPP_INFO(get_logger(),
                "MPC TCP loaded from robot_description: wrist3_link->tool0 z=%.4f m",
                wrist3_to_tool(2, 3));
            if (!ref_traj_waypoints_.empty()) {
            ...
            tcp_ready_ = false;
            RCLCPP_FATAL(get_logger(),"MPC TCP initialization failed: %s.", e.what());
            planning_scene_monitor_.reset();
    ...
    void controlLoop() {
        if (!tcp_ready_) {
            RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 1000,
                "MPC waits for robot_description TCP initialization.");
            return;
        }
    ...
    PrecheckResult stagePrecheck(RuntimeState& state, ControlCycleContext& ctx) {
        ...
        if (state.waiting_for_replan || state.terminal_hold) {
            publishHoldCommand(q_now);
            ctx.skip_remaining = true;
            return PrecheckResult{false, true};
        }

        if (state.waiting_dynamic_clearance) {
            const double dynamic_margin = computeObstacleMargin(
                q_now, collectDynamicObstacles());
            const double now_sec = steadyNowSec();
            if (dynamic_margin >= 0.0) {
                if (state.dynamic_clear_since_time_sec < 0.0) {
                    state.dynamic_clear_since_time_sec = now_sec;
                }
                if (now_sec - state.dynamic_clear_since_time_sec >=
                    params_.deadlock.dynamic_clear_confirm_sec) {
                    state.waiting_dynamic_clearance = false;
                    state.dynamic_clear_since_time_sec = -1.0;
                    state.local_recovery_start_time_sec = -1.0;
                    state.progress_stall_count = 0;
                    publishStatus("TRACKING");
                    RCLCPP_INFO(get_logger(),
                        "MPC resumes after dynamic obstacle clears: margin=%.4f", dynamic_margin);
                }
            } else {
                state.dynamic_clear_since_time_sec = -1.0;
            }
    ...
    PlanningResult stagePrepareAndSolve(RuntimeState& state, ControlCycleContext& ctx) {
        ...
        // 分离静态/动态裕度：重规划只能由静态前视路径失效触发。
        ctx.static_margin = computeObstacleMargin(ctx.q_now, ctx.obstacles.static_obs);
        ctx.dynamic_margin = computeObstacleMargin(ctx.q_now, ctx.obstacles.dynamic_obs);
        ctx.margin_all = std::min(ctx.static_margin, ctx.dynamic_margin);
        ...
        ctx.static_ahead_margin = computeReferenceAheadMargin(
            ctx.obstacles.static_obs, ctx.speed_ratio);
        ctx.dynamic_ahead_margin = computeReferenceAheadMargin(
            ctx.obstacles.dynamic_obs, ctx.speed_ratio);
        updateProgressStall();
        state.last_progress_s = last_progress_s_;

        if (state.reference_precheck_pending) {
            state.reference_precheck_pending = false;
            logClosestObstacleSample(ctx.q_now, ctx.obstacles.static_obs, "static");
            logClosestObstacleSample(ctx.q_now, ctx.obstacles.dynamic_obs, "dynamic");
            RCLCPP_INFO(get_logger(),
                "MPC_PRECHECK static_raw=%.4f dynamic_raw=%.4f static_ahead_raw=%.4f dynamic_ahead_raw=%.4f safe_dist=%.4f replan_budget=%d",
                ctx.static_margin + params_.safe_dist,
                ctx.dynamic_margin + params_.safe_dist,
                ctx.static_ahead_margin + params_.safe_dist,
                ctx.dynamic_ahead_margin + params_.safe_dist,
                params_.safe_dist,
                std::max(0, params_.deadlock.replan_max_per_goal - state.replan_count));
            if (ctx.margin_all < 0.0) {
                state.terminal_hold = true;
                state.mpc_active = false;
                state.has_reference = false;
                state.prev_u_sequence.clear();
                publishHoldCommand(ctx.q_now);
                RCLCPP_ERROR(get_logger(),
                    "MPC_PRECHECK rejected unsafe start: static_margin=%.4f dynamic_margin=%.4f",
                    ctx.static_margin, ctx.dynamic_margin);
                publishStatus("BLOCKED_UNSAFE_START");
                ctx.skip_remaining = true;
    ...
    CommandResult stagePublishCommand(RuntimeState& state, ControlCycleContext& ctx) {
        ...
        if (ctx.mpc_result.success) {
            ...
            dq_cmd = command_pipeline_.applyAvoidanceBiasAssist(
                ...
                ctx.margin_all, progress_stall_count_, params_, kinematics_, get_logger(), get_clock());
    ...
    void stageEvaluateDeadlock(RuntimeState& state, ControlCycleContext& ctx) {
        ...
        runDeadlockEvaluation(state, ctx);
    }
    ...
    double computeObstacleMargin(const VecN& q,
                                 const std::vector<Obstacle>& obstacles) const {
        if (obstacles.empty()) {
            return std::numeric_limits<double>::infinity();
        }
        return solver_->computeRobotObsMargin(q, obstacles) - params_.safe_dist;
    }

    void logClosestObstacleSample(const VecN& q,
                                  const std::vector<Obstacle>& obstacles,
                                  const char* category) const {
        if (obstacles.empty()) {
            return;
        }
        const auto points = kinematics_.samplePoints(q, params_.points_per_link);
        double min_distance = std::numeric_limits<double>::infinity();
        Vec3 closest_point = Vec3::Zero();
        Obstacle closest_obstacle;
        for (const auto& obstacle : obstacles) {
            for (const auto& point : points) {
                const double distance = SmoothBoxDistance::compute(
                    point, obstacle.center, obstacle.size, params_.kappa);
        ...
        RCLCPP_INFO(get_logger(),
            "MPC_PRECHECK %s closest: raw=%.4f point=[%.3f,%.3f,%.3f] "
            "box_center=[%.3f,%.3f,%.3f] box_size=[%.3f,%.3f,%.3f]",
            category, min_distance,
            closest_point.x(), closest_point.y(), closest_point.z(),
            closest_obstacle.center.x(), closest_obstacle.center.y(), closest_obstacle.center.z(),
            closest_obstacle.size.x(), closest_obstacle.size.y(), closest_obstacle.size.z());
    }

    double computeReferenceAheadMargin(const std::vector<Obstacle>& obstacles,
                                       double speed_ratio) const {
        if (obstacles.empty() || arc_follower_->getTotalLength() <= 1e-9) {
            return std::numeric_limits<double>::infinity();
        }
        const double ds_per_sec = params_.arc_follow.ds_physical_ratio *
            params_.dq_max.minCoeff() *
            std::max(speed_ratio, params_.arc_follow.min_speed_ratio);
        const double start_s = arc_follower_->getCurrentS() +
            params_.deadlock.reference_escape_prefix_sec * ds_per_sec;
        const double end_s = arc_follower_->getCurrentS() +
            params_.deadlock.reference_lookahead_sec * ds_per_sec;
        std::vector<double> queries;
        constexpr int kSamples = 5;
        queries.reserve(kSamples);
        for (int i = 0; i < kSamples; ++i) {
            const double alpha = static_cast<double>(i) / static_cast<double>(kSamples - 1);
            queries.push_back((1.0 - alpha) * start_s + alpha * end_s);
        }
        std::vector<VecN> q_samples;
        std::vector<VecN> tangents;
        arc_follower_->evalArcPath(queries, q_samples, tangents);
        double margin = std::numeric_limits<double>::infinity();
        for (const auto& q : q_samples) {
            margin = std::min(margin, computeObstacleMargin(q, obstacles));
        }
        return margin;
    }
    ...
    // 运行死锁评估引擎，更新相关计数器
    void runDeadlockEvaluation(RuntimeState& state, const ControlCycleContext& ctx) {
        DeadlockReplanEngine::Input in{
            arc_follower_->getCurrentS(),
            arc_follower_->getTotalLength(),
            ctx.static_margin,
            ctx.dynamic_margin,
            ctx.static_ahead_margin,
            ctx.dynamic_ahead_margin,
            steadyNowSec(),
            params_,
            state,
            get_logger(),
            [this](const std::string& s) { publishStatus(s); },
            [this, q_now = ctx.q_now]() { publishHoldCommand(q_now); }
        };
    ...
    void commandCallback(const std_msgs::msg::String::SharedPtr msg) {
        if (msg->data == "START") {
            ...
            replan_session_active_ = false;
            runtime_store_.mutate([](RuntimeState& state) {
                state.waiting_for_replan = false;
                state.waiting_dynamic_clearance = false;
                state.terminal_hold = false;
                state.replan_count = 0;
                state.last_replan_time_sec = -1.0;
                state.replan_cooldown_until_sec = -1.0;
            });
    ...
    void refTrajectoryCallback(const trajectory_msgs::msg::JointTrajectory::SharedPtr msg) {
        ...
        const bool same_goal = replan_session_active_ &&
            (path.waypoints.back() - replan_session_goal_q_).cwiseAbs().maxCoeff() <=
                params_.deadlock.goal_identity_tolerance_rad;
        if (!same_goal) {
            replan_session_active_ = true;
            replan_session_goal_q_ = path.waypoints.back();
        }

        // 同目标的新路径只重置跟踪/热启动；重规划预算属于整个目标会话。
        {
            ...
            runtime_store_.resetForNewTrajectory(same_goal);
            has_reference_ = true;
            mpc_active_ = true;
            goal_reported_ = false;
            ...
            if (tcp_ready_) {
                ...
                RCLCPP_WARN(get_logger(),
                    "Reference accepted before TCP initialization; delaying RViz path publication.");
            }
            publishStatus("TRACKING");
            RCLCPP_INFO(get_logger(), "Reference: %zu waypoints, same_goal=%d, goal=[%.2f,%.2f,%.2f,%.2f,%.2f,%.2f]",
                path.waypoints.size(),
                same_goal ? 1 : 0,
    ...
    RobotKinematics                   kinematics_;             ///< 当前 robot_description 的 TCP 运动学
    ...
    rclcpp::TimerBase::SharedPtr tcp_init_timer_; ///< 一次性TCP/MoveIt初始化定时器
    ...
    bool replan_session_active_ = false;
    VecN replan_session_goal_q_ = VecN::Zero();
