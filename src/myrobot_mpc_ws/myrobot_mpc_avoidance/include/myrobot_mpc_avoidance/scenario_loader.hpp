        ...
        if (path.empty()) {
            try {
                const auto share_dir =
                    ament_index_cpp::get_package_share_directory("myrobot_mpc_avoidance");
                ...
                const std::vector<std::filesystem::path> candidates{
                    cwd / "src" / "myrobot_mpc_avoidance" / "config" / "obstacle_stack.yaml",
                    cwd / "myrobot_mpc_avoidance" / "config" / "obstacle_stack.yaml",
