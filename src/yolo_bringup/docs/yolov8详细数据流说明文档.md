# yolo_bringup 重构与数据流说明

## 1. 文档目的

`yolo_bringup` 当前提供实机 YOLO-OBB 抓取入口、抓取状态机和少量场景工具。Gazebo/MoveIt 场景资产由 `myrobot_simulation` 维护。
...
- `perception.demo.launch.py`
  ...
  - 不再引用已删除的 `yolo_bringup/launch/gazebo.launch.py`。

当前保留的入口：

- `elongated_object_box_grasping = yolo_bringup.elongated_object_box_grasping_node:main`
...
Gazebo 视觉抓取入口 `myrobot_simulation/launch/visual_grasping_gazebo.launch.py` 同样加载 `config/visual_grasping.yaml`。
...
- `yolo_bringup/detector_common.py`
  - 模型路径解析。
  - YOLO 模型加载。
  - 相机内参缓存。
  - 深度图反投影。
  - 检测可视化绘制。
- `yolo_bringup/object_pose_estimator.py`
  - OBB yaw / RPY 估计。
  - 等价角度连续化。
  - 点云采样与鲁棒中心估计。
...
```bash
test -f yolo_bringup/docs/refactor_and_dataflow_guide.md
```

删除方案实施后的引用检查：

```bash
rg "pick_drop|pick_block|yolo_bringup_gazebo|movelt_cpp" yolo_bringup
```
...
```bash
python3 -m py_compile yolo_bringup/yolo_bringup/*.py yolo_bringup/yolo_bringup/*/*.py yolo_bringup/launch/*.launch.py
```

YAML 静态检查：

```bash
python3 -c "import yaml; yaml.safe_load(open('yolo_bringup/config/visual_grasping.yaml'))"
```

构建：

```bash
colcon build --packages-select yolo_perception yolo_bringup --symlink-install
```
...
ros2 launch yolo_bringup visual_grasping.launch.py
ros2 run yolo_perception yolo_detector_obb
ros2 run yolo_bringup elongated_object_box_grasping
```
...
- `yolo_bringup`：感知节点、旧版 elongated_object-box 抓取 demo、手动 abort 工具。
...
后续开发应尽量保持这个边界，避免 `yolo_bringup` 再次承担仿真、规划器、场景资产等跨包职责。
