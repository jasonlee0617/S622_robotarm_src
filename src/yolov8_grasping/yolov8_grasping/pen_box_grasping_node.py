...
class PenCubeBoxGraspingNode(Node):
    ...
    def __init__(self):
        ...
        self.target_stone_position: PointStamped | None = None
        self.target_pen_rpy: dict | None = None
        self.target_cube_rpy: dict | None = None
        self.target_stone_rpy: dict | None = None
        self.poses = {}
    ...
    def setup_detection_subscribers(self):
        ...
        self.create_subscription(PointStamped, "/stone_position_3d", self._on_stone_pos, qos_reliable_latest)
        ...
        self.create_subscription(Float32MultiArray, "/stone_rpy", self._on_stone_rpy, qos_reliable_latest)
    ...
    def _on_stone_pos(self, msg: PointStamped):
        self.target_stone_position = msg
    ...
    def _on_stone_rpy(self, msg: Float32MultiArray):
        if len(msg.data) >= 3:
            self.target_stone_rpy = {"roll": msg.data[0], "pitch": msg.data[1], "yaw": msg.data[2]}
        else:
            self.get_logger().warn("⚠ /stone_rpy format error")
    ...
    def setup_params(self):
        ...
        self.grasp_profile = {
            ...
            TargetType.STONE: {"roll": 0.0,"pitch": -180.0,"yaw_offset": -45.0,"above_z": 0.05,"grasp_z": 0.01,},
        }
    ...
    def _reset_task_cache(self):
        ...
        self.target_stone_position = None
        self.target_pen_rpy = None
        self.target_cube_rpy = None
        self.target_stone_rpy = None
    ...
    #     stone_ok = self._pair_valid(self.target_stone_position, self.target_stone_rpy, self.target_box_position)

    #     if pen_ok and not cube_ok:
    #         return TargetType.PEN
    #     if cube_ok and not pen_ok:
    #         return TargetType.CUBE
    #     if stone_ok and not pen_ok and not cube_ok:
    ...
    def select_target(self) -> TargetType | None:
        ...
        # 收集当前所有可抓取的目标
        available_targets = {}
        if pen_ok: available_targets["pen"] = TargetType.PEN
        if cube_ok: available_targets["cube"] = TargetType.CUBE
        if stone_ok: available_targets["stone"] = TargetType.STONE

        if not available_targets:
            return None

        # 1. 如果用户偏好的目标在视野内，直接优先抓它
        if self.preferred_target in available_targets:
            return available_targets[self.preferred_target]

        # 2. 如果偏好目标不在视野内，按照默认优先级抓取 (例如：pen > cube > stone)
        if "pen" in available_targets:
            return TargetType.PEN
        if "cube" in available_targets:
            return TargetType.CUBE
        if "stone" in available_targets:
            return TargetType.STONE
    ...
    def setup_poses_for_target(self, target: TargetType, obj_pos_base, box_pos_base):
        if target == TargetType.PEN:
            ...
            obj_name = "pen"
        elif target == TargetType.CUBE:
            obj_rpy = self.target_cube_rpy
            obj_name = "cube"
        elif target == TargetType.STONE:
            obj_rpy = self.target_stone_rpy
            obj_name = "stone"
        ...
        self.poses = {
            ...
            "box_above": self.pose_tools.make_pose(
                box_pos_base.x,
                box_pos_base.y,
                box_pos_base.z + self.place_offset,
                obj_roll, obj_pitch, obj_yaw,
            ),
            "descend_to_box": self.pose_tools.make_pose(
                box_pos_base.x,
                box_pos_base.y,
                box_pos_base.z + 0.07,  
                obj_roll, obj_pitch, obj_yaw,
    ...
    def control_loop(self):
        ...
        try:
            if self.current_state == TaskState.IDLE:
                ...
                if target is None:
                    self.get_logger().info(" Waiting for search target...")
                    return
                ...
                if target == TargetType.PEN:
                    ...
                    obj_msg = self.target_stone_position
                ...
                if self.move_to_pose(
                    ...
                    self.current_state = TaskState.DESCEND_TO_BOX
                else:
                    self.current_state = TaskState.ERROR

            elif self.current_state == TaskState.DESCEND_TO_BOX:
                # if not self.keepout.enabled:
                #     self.keepout.enable(self.Z_MIN)

                if self.move_to_pose(
                    self.poses["descend_to_box"],
                    cartesian=False,
...
#     GRASPING = "grasping"
#     LIFTING_TARGET = "lifting_target"
#     SEARCHING_BOX = "searching_box"             # 搜索盒子
...
#             return False
#         obj_age = self.msg_age_sec(obj_pos.header.stamp)
#         return obj_age < self.detection_timeout
    
#     # def select_target(
#     #     self,
#     #     TargetType,                      # enum passed in from caller (your TargetType)
#     #     pen_pos: PointStamped,
#     #     pen_rpy: dict,
#     #     cube_pos: PointStamped,
#     #     cube_rpy: dict,
...
#     #     return None
#     def select_target(self, TargetType, pen_pos, pen_rpy, cube_pos, cube_rpy, stone_pos, stone_rpy):
#         pen_ok = self.pair_valid(pen_pos, pen_rpy)
#         cube_ok = self.pair_valid(cube_pos, cube_rpy)
#         stone_ok = self.pair_valid(stone_pos, None)

#         # 1. 三者同时有效时，按偏好选择
#         if pen_ok and cube_ok and stone_ok:
#             if self.preferred_target == "cube":
#                 return TargetType.CUBE
#             elif self.preferred_target == "pen":
#                 return TargetType.PEN
#             elif self.preferred_target == "stone":
#                 return TargetType.STONE
#             # 如果没有特别偏好，可以按默认顺序（例如 CUBE）
#             return TargetType.CUBE

#         # 2. 两者有效的情况（如果你有偏好顺序，可以继续细化）
#         # 比如 pen+cube 有效，优先 cube
#         if pen_ok and cube_ok:
#             return TargetType.CUBE if self.preferred_target != "pen" else TargetType.PEN
#         if pen_ok and stone_ok:
#             return TargetType.STONE if self.preferred_target == "stone" else TargetType.PEN
#         if cube_ok and stone_ok:
#             return TargetType.CUBE if self.preferred_target != "stone" else TargetType.STONE
...
#         return None
#     # ---------------- Pose generation (pen/cube identical) ----------------
#     def setup_poses_for_target(self, target: TargetType, obj_pos_base):
#         if target == TargetType.PEN:
#             obj_rpy = self.target_pen_rpy
#             obj_name = "pen"
#         else:
...
#                 target = self.select_target()
#                 if target is None:
#                     self.get_logger().info("⏳ Waiting for pen or cube......")
#                     return
...
#                 self.setup_poses_for_target(target, obj_pos_base)
...
#                 box_msg = self.target_box_position
#                 # ↑ 盒子点消息
#                 box_pos_base = self.tf_tools.camera_point_to_base(box_msg)  # ↑ 把 box 点从 camera frame 变换到 base frame
                
#                 if box_pos_base is None:
#                     self.get_logger().warn("⚠ box searching failed, keep searching...")
#                     self.current_state = TaskState.ERROR
#                     return    
#                 place_z = float(box_pos_base[2] + self.place_offset)
#                 target_box_pose = self.pose_tools.make_pose(float(box_pos_base[0]),float(box_pos_base[1]),place_z,0.0,-180.0,0.0)
#                 if self.move_to_pose(
#                     target_box_pose,
#                     cartesian=False,
