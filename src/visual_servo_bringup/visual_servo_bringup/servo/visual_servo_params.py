...
class ServoRuntimeConfig:
    ...
    def from_node(cls, node) -> "ServoRuntimeConfig":
        ...
        cfg = cls(
            ...
            nladrc_wc_xy=float(param(node, "nladrc_wc_xy", 12.5)),
            ...
            nladrc_alpha_obs_xy=float(param(node, "nladrc_alpha_obs_xy", 0.98)),
            nladrc_alpha_obs2_xy=float(param(node, "nladrc_alpha_obs2_xy", 0.95)),
            ...
            nladrc_obs_transition_xy=float(param(node, "nladrc_obs_transition_xy", 0.012)),
            nladrc_obs_transition_z=float(param(node, "nladrc_obs_transition_z", 0.002)),
            nladrc_z2_clip_xy=float(param(node, "nladrc_z2_clip_xy", 0.14)),
            nladrc_z2_clip_z=float(param(node, "nladrc_z2_clip_z", 0.08)),
            nladrc_u_fb_clip_xy=float(param(node, "nladrc_u_fb_clip_xy", 0.24)),
            ...
            nladrc_z2_decay_gain_xy=float(param(node, "nladrc_z2_decay_gain_xy", 3.0)),
            nladrc_z2_gain_xy=float(param(node, "nladrc_z2_gain_xy", 1.0)),
            nladrc_ff_mix_gain=float(param(node, "nladrc_ff_mix_gain", 0.30)),
            ...
            mpc_horizon=int(param(node, "mpc_horizon", 16)),
            mpc_tau=float(param(node, "mpc_tau", 0.010)),
            mpc_delay_sec=float(param(node, "mpc_delay_sec", 0.005)),
            mpc_delay_steps=int(param(node, "mpc_delay_steps", 1)),
            mpc_q_e=float(param(node, "mpc_q_e", 280.0)),
            mpc_q_v=float(param(node, "mpc_q_v", 2.0)),
            mpc_q_terminal=float(param(node, "mpc_q_terminal", 420.0)),
            mpc_r_u=float(param(node, "mpc_r_u", 0.20)),
            mpc_r_du=float(param(node, "mpc_r_du", 2.5)),
            mpc_u_max=float(param(node, "mpc_u_max", 0.24)),
            mpc_du_max=float(param(node, "mpc_du_max", 0.018)),
            mpc_norm_clip=float(param(node, "mpc_norm_clip", 0.24)),
