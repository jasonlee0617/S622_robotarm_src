...
class PenCubeBoxGraspingNode(Node):
    ...
    def setup_moveit(self):
        try:
            ...
            self.moveit2_arm.planner_id = "RRTConnectFas"
            self.moveit2_arm.max_step_size = 0.25
    ...
    def setup_params(self):
        ...
        self.WRIST_WEIGHT = 100.0
        self.WRIST_JOINT_INDICES = (2, 3, 4) #约束关节

        self.grasp_profile = {
        TargetType.PEN: {"roll": 0.0,"pitch": -180.0,"yaw_offset": -180.0,"above_z": 0.03,"grasp_z": 0.01,},
        TargetType.CUBE: {"roll": 0.0,"pitch": -180.0,"yaw_offset": -180.0,"above_z": 0.05,"grasp_z": 0.01,},
        }
    ...
    def move_to_pose(self, target_pose, cartesian=False, action_name="move", max_velocity=0.05, max_acceleration=0.05):
        ...
        try:
            ...
            ok = self.abort.wait_idle_or_abort(self.moveit2_arm, action_name, timeout_sec=120.0)
            if not ok:
    ...
    def control_gripper(self, open_gripper=True):
        ...
        positions = [0.0295, -0.0295] if open_gripper else [0.01, -0.01]
        try:
