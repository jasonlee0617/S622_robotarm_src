...
- `gazebo_launch_dataflow.md`
- `gazebo_launch_refactor.md`
...
### 4.2 `gazebo_yolo.launch.py`

复用 `gazebo.launch.py` 的基础栈，并额外按开关启动：
...
```text
gazebo_launch/docs/graspnet_visual_grasping_table说明文档.md
```
...
3. 任何算法调参变更都要在 `fairino_planning_core` 参数源统一维护。
...
`fairino_mpc_avoidance/launch/mpc_planning_demo.launch.py` 在启动时根据 `robot_profile` 读取上述字段，并自动注入：
...
```bash
ros2 launch fairino_mpc_avoidance mpc_planning_demo.launch.py robot_profile:=fairino3_v6
```
