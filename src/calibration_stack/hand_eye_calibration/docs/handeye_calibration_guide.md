...
```text
calibration_stack/hand_eye_calibration/docs/auto_calibration_collector_design.md
```

当你要持续整改 collector，建议同时按固定格式整理诊断数据包，说明和一键采集脚本见：

```text
calibration_stack/hand_eye_calibration/docs/auto_calibration_diagnostic_bundle.md
calibration_stack/hand_eye_calibration/scripts/collect_auto_calibration_diagnostics.sh
```
