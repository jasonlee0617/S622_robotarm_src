...
用法:
    ...
    python3 scripts/generate_acados_solver.py   # 从包根目录运行
"""
import os
import re
import sys
...
# 确保从包根目录运行，c_generated_code/ 生成在正确位置
...
def create_model():
    ...
    eq = q - p_qRef
    ev = dq - p_qdRef
    apf_cost = p_obsW * (ca.dot(p_obsGrad, eq) + p_obsQuad * ca.dot(eq, eq))

    model.cost_expr_ext_cost = (
        p_trackW * ca.dot(eq, eq) +
        p_velW * ca.dot(ev, ev) +
    ...
    cbf_val = ca.dot(p_cbfGradQ, dq) + p_cbfVobs + p_cbfGamma * p_cbfH + eps_cbf
...
def generate_solver():
    ...
    # 生成 C 代码到 c_generated_code/，json 存档到 scripts/
    json_path = os.path.join(_PKG_ROOT, 'scripts', 'acados_ocp_fairino_mpc.json')
    solver = AcadosOcpSolver(ocp, json_file=json_path)
    _patch_generated_solver_for_current_acados()

    print("acados C 代码生成完成！")
    print(f"  输出目录: {os.path.join(_PKG_ROOT, 'c_generated_code')}")
