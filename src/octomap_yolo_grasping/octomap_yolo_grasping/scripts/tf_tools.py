...
class TfTools:
    """
    - 内部创建 tf Buffer/Listener
    - 用 timer 检测 base_frame <-> camera_frame 是否可用
    - 提供把 PointStamped 从 camera frame 变换到 base frame 的函数
    """
    ...
    def transform_point(self, point_stamped, target_frame: str, timeout_sec: float = 0.2):
        """
        通用点变换：PointStamped(header.frame_id=source) -> target_frame
        返回 geometry_msgs/Point（或 None）
        """
        if not self.ready:
            return None
