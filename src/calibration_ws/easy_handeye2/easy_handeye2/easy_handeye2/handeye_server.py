...
from easy_handeye2 import next_snapshot_id, resolve_storage_directory
...
class HandeyeServer(rclpy.node.Node):
    def __init__(self):
        ...
        self.storage_directory = self.declare_parameter('storage_directory', '').value
        self._storage_path = resolve_storage_directory(self.storage_directory)
        self._pending_snapshot_id = None

        self.get_logger().info(
            f'Read parameters for calibration "{self.parameters.name}"; '
            f'storage_directory={self._storage_path or "legacy default"}'
        )

        self.sampler = HandeyeSampler(
            self,
    ...
    def _clear_pending_snapshot(self):
        self._pending_snapshot_id = None
    ...
    def save_calibration(self, _, response: ehm.srv.SaveCalibration.Response):
        ...
        if self.last_calibration:
            try:
                snapshot_id = self._pending_snapshot()
                samples_filepath = None
                if snapshot_id is not None:
                    # RQT's Save button invokes only SaveCalibration.  Persist the
                    # corresponding sample set here so every timestamped result has
                    # a matched .calib/.samples pair.
                    samples_filepath = self.sampler.save_samples(snapshot_id)
                ...
                if samples_filepath is not None:
                    self.get_logger().info(f'Matching samples saved to {samples_filepath}')
                response.success = True
