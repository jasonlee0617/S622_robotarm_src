...
```bash
$HOME/tmp/case_YYYYMMDD_HHMM/commands/run_launch.sh
```
...
这些默认值优先取自 `auto_calibration_collector.yaml`，缺失时才回落到 `calibration_gazebo.launch.py`；
`gazebo_yolo.launch.py` 仅保留 standalone fallback（60/640/480），不作为校准链路默认值。
...
```bash
$HOME/tmp/case_YYYYMMDD_HHMM/commands/run_collector.sh
```
