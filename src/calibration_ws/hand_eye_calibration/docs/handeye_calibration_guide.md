...
Step 1: calibrate.launch.py  (标定数据采集 + 求解 + 保存)
  ...
  │    · Compute → 求解手眼变换
  │    · Save → 保存匹配的时间戳 `.calib` / `.samples` 文件
  │
...
```text
calibration_ws/hand_eye_calibration/docs/auto_calibration_collector_design.md
```

当你要持续整改 collector，建议同时按固定格式整理诊断数据包，说明和一键采集脚本见：

```text
calibration_ws/hand_eye_calibration/docs/auto_calibration_diagnostic_bundle.md
calibration_ws/hand_eye_calibration/scripts/collect_auto_calibration_diagnostics.sh
```
...
真实机械臂的眼在手上自动标定入口（默认待命，不会自动运动）：

```bash
ros2 launch hand_eye_calibration real_eye_in_hand_auto_calibration.launch.py
ros2 service call /auto_calibration_collector/start std_srvs/srv/Trigger {}
```

如已确认现场安全且希望启动后自动执行一次采集，传入 `auto_start:=true`。Gazebo 通过
`auto_collect:=true auto_start:=true` 启用同样行为。

如果你是单独手工启动 collector，而不是通过 launch 拉起，直接运行即可（当前 collector 默认 `use_sim_time=true`）：

```bash
ros2 run hand_eye_calibration auto_calibration_collector.py
```

如果不是 Gazebo 仿真，而是需要 wall time，再显式传 `--ros-args -p use_sim_time:=false`。
...
此工作区将标定结果保存到
`$HOME/fairino_robotarm/src/calibration_ws/hand_eye_calibration/calib/sim` 或
`.../calib/real`。每次结果都以
`robot_calibration_YYYYMMDD_HHMMSS.{calib,samples}` 的匹配文件对保存。
确认目录存在且有写入权限：
```bash
ls -la "$HOME/fairino_robotarm/src/calibration_ws/hand_eye_calibration/calib/real"
```
