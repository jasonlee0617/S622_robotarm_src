...
private:
  ...
  std::mutex _io_mutex;
  std::array<double, 6> _latest_command{};
  std::array<double, 6> _latest_state{};
  std::array<double, 6> _latest_velocity{};
  std::atomic<int64_t> _last_feedback_ns{0};
  std::atomic<int64_t> _last_command_ns{0};
  std::atomic<uint64_t> _servo_cycles{0};
  std::atomic<uint64_t> _cycle_overruns{0};
  std::atomic<uint64_t> _servo_failures{0};
  std::atomic<uint64_t> _feedback_failures{0};
  std::atomic<uint64_t> _speed_feedback_failures{0};
  double _servoj_cmd_t{0.008};
  int _realtime_state_period_ms{10};
