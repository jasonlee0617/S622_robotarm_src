...
NMPCSolver::~NMPCSolver() {
    if (capsule_) {
        s622arm_nmpc_acados_free(capsule_);
        s622arm_nmpc_acados_free_capsule(capsule_);
    }
}
...
bool NMPCSolver::initialize(const MPCParams& params) {
    ...
    // 创建 acados capsule
    capsule_ = s622arm_nmpc_acados_create_capsule();
    int status = s622arm_nmpc_acados_create(capsule_);
    if (status != 0) {
        return false;
    }

    // 获取内部接口指针
    nlp_config_ = s622arm_nmpc_acados_get_nlp_config(capsule_);
    nlp_dims_   = s622arm_nmpc_acados_get_nlp_dims(capsule_);
    nlp_in_     = s622arm_nmpc_acados_get_nlp_in(capsule_);
    nlp_out_    = s622arm_nmpc_acados_get_nlp_out(capsule_);
    nlp_solver_ = s622arm_nmpc_acados_get_nlp_solver(capsule_);
    nlp_opts_   = s622arm_nmpc_acados_get_nlp_opts(capsule_);
...
void NMPCSolver::resetSolverMemory(bool reset_qp_solver_mem) {
    ...
    s622arm_nmpc_acados_reset(capsule_, reset_qp_solver_mem ? 1 : 0);
}
...
MPCResult NMPCSolver::solve(const MPCSolveContext& ctx)
{
    ...
    for (int k = 0; k < N; ++k) {
        ...
        // 更新 acados 阶段参数
        s622arm_nmpc_acados_update_params(capsule_, k, p_vec.data(), np);
    ...
    // 终端阶段：使用 terminal_weight，无 APF/CBF 耦合
    {
        ...
        s622arm_nmpc_acados_update_params(capsule_, N, p_vec.data(), np);
    }

    // 4. 调用 acados 求解器
    int acados_status = s622arm_nmpc_acados_solve(capsule_);
