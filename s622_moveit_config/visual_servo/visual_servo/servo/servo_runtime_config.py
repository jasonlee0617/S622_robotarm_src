...
class ServoRuntimeConfig:
    ...
    servo_handoff_zero_twist_dt: float
    servo_handoff_ee_speed_tol: float
    servo_handoff_joint_speed_tol: float
    servo_handoff_stable_sec: float
    servo_handoff_timeout_sec: float
    ...
    def from_node(cls, node) -> "ServoRuntimeConfig":
        ...
        cfg = cls(
            ...
            servo_handoff_zero_twist_dt=float(_declare_get(node, "servo_handoff_zero_twist_dt", 0.01)),
            servo_handoff_ee_speed_tol=float(_declare_get(node, "servo_handoff_ee_speed_tol", 0.003)),
            servo_handoff_joint_speed_tol=float(_declare_get(node, "servo_handoff_joint_speed_tol", 0.01)),
            servo_handoff_stable_sec=float(_declare_get(node, "servo_handoff_stable_sec", 0.08)),
            servo_handoff_timeout_sec=float(_declare_get(node, "servo_handoff_timeout_sec", 0.5)),
    ...
    def validate(self) -> None:
        ...
        if self.servo_handoff_zero_twist_dt < 0.0:
            raise RuntimeError("servo_handoff_zero_twist_dt must be >= 0")
        if self.servo_handoff_ee_speed_tol <= 0.0 or self.servo_handoff_joint_speed_tol <= 0.0:
            raise RuntimeError("servo_handoff speed tolerances must be > 0")
        if self.servo_handoff_stable_sec < 0.0 or self.servo_handoff_timeout_sec <= 0.0:
            raise RuntimeError("servo_handoff timing values are invalid")
