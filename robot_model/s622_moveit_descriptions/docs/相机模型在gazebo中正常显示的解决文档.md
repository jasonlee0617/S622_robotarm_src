...
| 层次 | 问题 | 后果 |
|---|---|---|
| 1. `package://` 路径 | `robot_description_with_package_paths()` 只解析了 `s622_moveit_descriptions` 包路径 | Gazebo 收不到 mesh 文件系统路径 |
...
combined.export('$(ros2 pkg prefix s622_moveit_descriptions)/share/s622_moveit_descriptions/meshes/d435.stl')
...
**文件**: `s622_moveit_descriptions/urdf/camera/_d435.urdf.xacro`

```xml
<!-- Mesh 引用改为本地 STL（第 78 行） -->
<mesh filename="package://s622_moveit_descriptions/meshes/d435.stl" />
...
**文件**: `s622_moveit_descriptions/urdf/camera/camera.xacro`

```xml
<!-- Include 从系统包改为本地宏 -->
<!-- <xacro:include filename="$(find realsense2_description)/urdf/_d435.urdf.xacro" /> -->
<xacro:include filename="$(find s622_moveit_descriptions)/urdf/camera/_d435.urdf.xacro" />
...
3. 放入 `s622_moveit_descriptions/meshes/`
4. 在 URDF 中引用：`<mesh filename="package://s622_moveit_descriptions/meshes/xxx.stl"/>`
